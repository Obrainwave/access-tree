...
@section('content')
    <div class="modern-resource-page">
        ...
        <div class="form-card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('accesstree.admin.users.update', $item->id) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            @if ($fieldConfig['type'] === 'text' || $fieldConfig['type'] === 'email')
                                <input type="{{ $fieldConfig['type'] }}"
                                    ...
                                    name="{{ $fieldName }}" value="{{ old($fieldName, $item->$fieldName) }}"
                                    placeholder="{{ $fieldConfig['placeholder'] ?? '' }}"
                                    @if ($fieldConfig['required'] ?? false) required @endif>
                            @elseif($fieldConfig['type'] === 'checkbox-group')
                                @php
                                    $selectedRoles = $item->roles->pluck('id')->toArray();
                                @endphp
                                        ...
                                        <div class="col-md-4 col-sm-6 mb-2">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="{{ $fieldName }}[]"
                                                    ...
                                                    {{ in_array($value, old($fieldName, $selectedRoles)) ? 'checked' : '' }}>
